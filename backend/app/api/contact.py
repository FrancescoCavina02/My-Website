...
async def submit_contact(
    message: ContactMessage, request: Request, db: AsyncSession = Depends(get_db)
):
    """
...
@router.get("/messages")
async def get_messages(db: AsyncSession = Depends(get_db), _admin: dict = Depends(verify_admin)):
    """
    ...
    return {"total": len(messages_list), "messages": messages_list}
