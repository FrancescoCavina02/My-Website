...
class TreeNode:
    ...
    def __init__(self, note: Note, is_root: bool = False, is_leaf: bool = False, depth: int = 0):
        self.note = note
...
class TreeParser:
    ...
    def is_root_note(self, note: Note) -> bool:
        ...
        if (
            ...
            or filename.startswith("notes-")
            or filename.startswith("a ")
        ):
            return True

        return False

    def find_note_by_link_text(
        self, link_text: str, base_category: str, base_book: Optional[str] = None
    ) -> Optional[Note]:
        """
    ...
    def _build_tree_recursive(
        self, parent_node: TreeNode, category: str, book: Optional[str], visited: Set[str]
    ):
        ...
        for link_text in wiki_links:
            ...
            # Create child node
            child_node = TreeNode(note=child_note, is_root=False, depth=parent_node.depth + 1)
        ...
        for category in sorted(categories):
            ...
            for book in sorted(books):
                ...
                if not root_notes and book_notes:
                    ...
                    logger.info(
                        f"No root note found for '{book}', using '{root_notes[0].title}' as fallback"
                    )
    ...
    def get_navigation_context(self, note: Note, tree: TreeNode) -> Dict:
        ...
        if node is None:
            return {"breadcrumbs": [], "siblings": [], "children": [], "parent": None}
        ...
        # Get children
        children = [{"id": child.note.id, "title": child.note.title} for child in node.children]

        # Get parent
        parent = None
        if node.parent:
            parent = {"id": node.parent.note.id, "title": node.parent.note.title}
