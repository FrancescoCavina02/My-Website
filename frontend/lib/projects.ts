...
export const projects: Project[] = [
  {
    ...
    description: "Full-stack portfolio powered by a FastAPI backend that live-parses an Obsidian vault (1,649 notes → 1,772 semantic chunks) into a REST API with multi-LLM RAG chat and SSE streaming, deployed on Render + Netlify.",
  ...
  },
  {
    ...
    description: "Production-deployed RAG system that semantically searches 1,649 personal Obsidian notes (~300K words) and delivers grounded, cited responses via GPT-4 Turbo.",
    longDescription: "Five-stage RAG pipeline built from scratch: (1) Obsidian vault parser with WikiLink graph extraction; (2) structure-aware chunking (800-token chunks, 150-token overlap); (3) sentence-transformers/all-MiniLM-L6-v2 → 384-dim L2-normalised vectors in ChromaDB HNSW; (4) hybrid retrieval: 70% cosine similarity + 20% BM25 keyword overlap + 10% WikiLink density, recovering ~12% additional recall on named-entity queries vs. dense-only; (5) GPT-4 Turbo with citation-injected structured prompts, token-by-token SSE streaming. Multi-LLM abstraction (OpenAI, Ollama/Llama 3.1, Anthropic, Google) via shared abstract base class. Evaluated across 20 queries: GPT-4 Turbo achieves 4.7/5 citation accuracy, 4.6/5 contextual grounding.",
    category: "ai",
    technologies: ["Python", "FastAPI", "ChromaDB", "sentence-transformers", "OpenAI GPT-4 Turbo", "Llama 3.1", "Claude 3 Sonnet", "SSE Streaming", "BM25", "HNSW", "Next.js", "RAG"],
